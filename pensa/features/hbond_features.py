...
def get_cavity_bonds(structure_input, xtc_input, atomgroup, site_IDs, 
    ...
    if write is True:
        if not os.path.exists('site_hbonds/'):
            os.makedirs('site_hbonds/')
        p = u.select_atoms("protein")
        pdb_outname = 'site_hbonds/' + out_name + "_Sites.pdb"
        ...
        # p.write(pdb_outname)
        "WRITE OUT DOES NOT YET ADD CAVITY LOCATIONS"
    ...
    for site_no in site_IDs:
        ...
        # for frame_no in tqdm(range(1000)):       
            u.trajectory[frame_no]
            radius = ' 3.5'
            atomgroup_IDS = u.select_atoms('byres (name ' + atomgroup + ' and point ' + point_str + radius + ')').resids
        ...
        # for frame_no in tqdm(range(1000)):       
            ...
            if len(site_resid)==1:        
                ...
                proxprotatg = 'protein and around 3.5 resid ' + str(site_resid[0])
                O_site = '(resid ' + str(site_resid[0]) + ' and type OW)'
                H_site = '(resid ' + str(site_resid[0]) + ' and type HW)'
                ...
                # bonds formed by the water oxygen
                    O_bonds = '( '+protein_hydrogens_sel+' or (resname ASP and name HD2) or (resname GLU and name HE2)) and around 3.5 '+O_site
                else:
                    O_bonds = '(resname ASP and name HD2) and around 3.5 '+O_site
            ...
            ## Featurize water with highest pocket occupation (if multiple waters in pocket)
                ...
                proxprotatg = 'protein and around 3.5 resid ' + str(freq_count[-1][1])
                O_site = '(resid ' + str(freq_count[-1][1]) + ' and type OW)'
                H_site = '(resid ' + str(freq_count[-1][1]) + ' and type HW)'
        ...
        if write is True:    
            np.savetxt('site_hbonds/' + out_name + O_site_pdb_id + '_names.txt', np.array(bondouts[0][0],dtype=object), fmt='%s')
            np.savetxt('site_hbonds/' + out_name + O_site_pdb_id + '_data.txt',  np.array(bondouts[0][1],dtype=object), fmt='%s')
            np.savetxt('site_hbonds/' + out_name + H_site_pdb_id + '_names.txt', np.array(bondouts[1][0],dtype=object), fmt='%s')
            np.savetxt('site_hbonds/' + out_name + H_site_pdb_id + '_data.txt',  np.array(bondouts[1][1],dtype=object), fmt='%s')
    
        
    return 'Cavity H-Bond Analysis Complete'
